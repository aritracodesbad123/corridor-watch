...
**Corridor Watch** is an end-to-end cross-border payment fraud investigation console, explainability layer, and compliance AI reasoning engine built for JAPAC remittance corridors. The current POC includes a deterministic evaluation harness and explicit RBAC boundaries so the AI layer can be judged on reliability, not only demo breadth.
...
The POC has explicit RBAC enforcement at the API boundary. Demo mode accepts request headers so the local console remains self-contained, while production mode can use API-key-to-role mappings via `CORRIDOR_WATCH_API_KEYS`.
...
### Production-style API key mapping

Set a JSON object whose keys are secrets and values contain the server-controlled identity/role:

```bash
export CORRIDOR_WATCH_API_KEYS='{"key-1":{"analyst_id":"lead_michael","role":"fiu_lead"}}'
```

When this variable is present, requests without a valid `X-API-Key` receive `401`; a client-supplied analyst ID/role is ignored. The API-key mode is a POC boundary, not a substitute for Google Cloud Identity, Entra ID, OAuth/OIDC, or workload identity in production.

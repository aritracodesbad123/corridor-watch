...
app = FastAPI(title="Corridor Watch", version="1.4.0", lifespan=lifespan)
...
def health():
    return {
        ...
        "phases": ["1", "2", "3"],
        "auth_mode": "api_key" if os.getenv("CORRIDOR_WATCH_API_KEYS") else "demo_header",
    }


@app.get("/api/alerts")
def list_alerts(auth: AuthContext = Depends(require("alerts:read"))):
    con = connect()
    rows = con.execute(
...
def verify_doc(txn_id: str, body: DocVerifyRequest | None = None, auth: AuthContext = Depends(require("document:verify"))):

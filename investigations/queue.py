...
def mark_running(queue_id: str, worker_id: str) -> None:
    _set(queue_id, "RUNNING", worker_id=worker_id)
...
def mark_failed(queue_id: str, error: str, *, retry: bool = True, max_attempts: int = 3) -> str:
    con = connect()
    ...
    if retry and attempts < max_attempts:
        ...
        con.execute(
            """UPDATE investigation_queue
               ...
               WHERE queue_id=?""",
            (status, error[:500], now, now, queue_id),
        )
    else:
        status = "DEAD_LETTER"
        con.execute(
